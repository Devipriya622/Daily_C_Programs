...
int findSmallestMissingPositive(int orderNumbers_count, int* orderNumbers) {
    ...
    for(int i = 0; i < orderNumbers_count; i++) {
        if(orderNumbers[i] <= 0 || orderNumbers[i] > orderNumbers_count) {
            orderNumbers[i] = orderNumbers_count + 1;
        }
    }

    for(int i = 0; i < orderNumbers_count; i++) {
        int val = abs(orderNumbers[i]);
        if(val >= 1 && val <= orderNumbers_count) {
            if(orderNumbers[val - 1] > 0) {
                orderNumbers[val - 1] = -orderNumbers[val - 1];
            }
        }
    }
