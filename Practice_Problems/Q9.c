...
int main()
{
    int links_rows = parse_int(ltrim(rtrim(readline())));

    int links_columns = parse_int(ltrim(rtrim(readline())));

    int** links = malloc(links_rows * sizeof(int*));

    for (int i = 0; i < links_rows; i++) {
        *(links + i) = malloc(links_columns * (sizeof(int)));

        char** links_item_temp = split_string(rtrim(readline()));

        for (int j = 0; j < links_columns; j++) {
            int links_item = parse_int(*(links_item_temp + j));

            *(*(links + i) + j) = links_item;
        }
    }

    int n = parse_int(ltrim(rtrim(readline())));

    int result = countIsolatedCommunicationGroups(links_rows, links_columns, links, n);
