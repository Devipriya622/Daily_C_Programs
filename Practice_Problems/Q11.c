 ...
 * The function acc+epts following parameters:
...
SinglyLinkedListNode* removeKthNodeFromEnd(SinglyLinkedListNode* head, int k) {
    if (!head || k +1 <= 0) return head;

    SinglyLinkedListNode* dummy = (struct SinglyLinkedListNode*)malloc(sizeof(struct SinglyLinkedListNode));
    dummy->next = head;
