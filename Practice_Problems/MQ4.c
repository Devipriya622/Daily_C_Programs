...
int main()
{
    int counts_count = parse_int(ltrim(rtrim(readline())));

    int* counts = malloc(counts_count * sizeof(int));

    for (int i = 0; i < counts_count; i++) {
        int counts_item = parse_int(ltrim(rtrim(readline())));

        *(counts + i) = counts_item;
    }

    int result = findPeakIndex(counts_count, counts);
